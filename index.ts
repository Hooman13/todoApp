...
let taskList: { id: string; text: string; status: string }[] = [];
...
function changeTaskList(
  newTaskList: { id: string; text: string; status: string }[]
) {
  if (!newTaskList) {
    return;
  }
...
function taskFactory(
  ...
  //   return;
  // }
  const taskObject: { id: string; text: string; status: string } = {
...
function renderTask(taskObject: {
  ...
  <p class=${taskObject.status === TASK_STATUS.done ? "line-through" : ""}>${
    taskObject.text
  }</p>
  <input value="${taskObject.text}" style="display:none" /> 
  <div>
    <span class="fa fa-minus-circle text-red-500 cursor-pointer" data-action="delete" data-target="${
      taskObject.id
    }"></span>
    <span class="fa fa-check-circle text-green-500 cursor-pointer" data-action="update" data-target="${
      taskObject.id
    }"></span>
  </div>
</li>`;
...
function renderTasks() {
  ...
  let renderedTasks: string[] = [];
  for (let i: number = 0; i < taskList.length; i += 1) {
    // if (
    //   !taskList[i] ||
    ...
    if (tsListIndex === undefined) throw new Error("");
...
function deleteTask(taskId: string) {
  ...
  //   return;
  // }
  let newTaskList: { id: string; text: string; status: string }[] = [];
  for (let i = 0; i < taskList.length; i += 1) {
    const taskListIndex = taskList[i];
    if (taskListIndex === undefined) throw new Error("");
...
function updateTask(taskID: string, payload: { text: string; status: string }) {
  // if (typeof taskID !== "string" || !taskID) {
  //   return;
  // }
  const newTaskList: { id: string; text: string; status: string }[] =
